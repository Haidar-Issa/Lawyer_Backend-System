...
public class UnifiedFailureAnalyze extends AbstractFailureAnalyzer<Throwable> {

    @Override
    protected FailureAnalysis analyze(Throwable rootFailure, Throwable cause) {

        if (cause instanceof MissingDocumentStorageException) {
            String description = String.format(
                    "فشل إقلاع نظام المحاماة! مسار حفظ ملفات وعقود الموكلين غير صالح أو لا يملك صلاحية كتابة: [%s]",
                    ((MissingDocumentStorageException) cause).getStoragePath()
            );
